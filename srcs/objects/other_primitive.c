...
t_equation	tp_init_eq(t_obj *obj, t_vector o, t_vector v, t_vector dir)
{
	t_equation	eq;

	eq.a = vec_dot(&v, &v) - vec_dot(&v, &dir) * vec_dot(&v, &dir);
	eq.b = 2 * vec_dot(&v, &o) - vec_dot(&v, &dir) *
	(vec_dot(&o, &dir) + 2 * obj->size);
	eq.c = vec_dot(&o, &o) - vec_dot(&o, &dir) *
	(vec_dot(&o, &dir) + 4 * obj->size);
